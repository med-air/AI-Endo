################################################################################
# Refer to https://github.com/mahmoudnafifi/WB_color_augmenter.git
...
class WBEmulator:
  def __init__(self):
    ...
    self.features = np.load('utils/params/features.npy')
    # mapping functions to emulate WB effects
    self.mappingFuncs = np.load('utils/params/mappingFuncs.npy')
    # weight matrix for histogram encoding
    self.encoderWeights = np.load('utils/params/encoderWeights.npy')
    # bias vector for histogram encoding
    self.encoderBias = np.load('utils/params/encoderBias.npy')
  ...
  def single_image_processing(self, in_img, outNum=1):
    """Applies the WB emulator to a single image in_img.
    Only one image is generated now.
    """
    assert (outNum <= 10)
    # I = Image.fromarray(in_img)  # read the image
    outImgs, wb_pf = self.generateWbsRGB(in_img, outNum)
    outImgs = [outImg for outImg in outImgs]

    return outImgs[0]  # get the ith output image
